    ...
    $(async fn $method:ident (&$self:ident $(,$req:ident : $reqType:ty)* ) -> Result<$resType:ty>  { $($code:tt)* })*) => {
        impl $name {
            $(
                ...
                async fn $method (&$self $(,$req : $reqType)* ) -> Result<$resType,fusen_common::FusenError> { $($code)* }
            )*
            ...
            async fn prv_invoke (&self, mut param : fusen_common::FusenMsg) -> fusen_common::FusenMsg {
                $(if &param.method_name[..] == stringify!($method) {
                    ...
                    $(
                        ...
                        if let Err(err) = result {
                            param.res = Err(fusen_common::FusenError::Server(err.to_string()));
                            return param;
                        }
                    ...
                    param.res = match res {
                        Ok(res) => {
                            ...
                            match res {
                                Ok(res) => Ok(res),
                                Err(err) => Err(fusen_common::FusenError::Server(err.to_string()))
                            }
                        },
                        Err(info) => Err(info)
                    };
                ...
                param.res = Err(fusen_common::FusenError::Server(format!("not find method by {}",param.method_name)));
                return param;
            }
        }
...
macro_rules! fusen_client {
    ...
    $(async fn $method:ident (&$self:ident $(,$req:ident : $reqType:ty)* ) -> Result<$resType:ty> )*) => {
        impl $name {
            $(
                #[allow(non_snake_case)]
                async fn $method (&$self $(,$req : $reqType)*) -> Result<$resType,fusen_common::FusenError> {
                    let mut req_vec : Vec<String> = vec![];
                    $(
                        let mut res_str = serde_json::to_string(&$req);
                        if let Err(err) = res_str {
                            return Err(fusen_common::FusenError::Client(err.to_string()));
                        }
                    ...
                    let msg = fusen_common::FusenMsg::new(
                        ...
                        req_vec,
                        Err(fusen_common::FusenError::Null)
                    );
                    let res : Result<$resType,fusen_common::FusenError> = $cli.invoke::<$resType>(msg).await;
                    return res;
                }
            )*
        }
