...
pub fn debug(item: TokenStream, runtime: proc_macro2::TokenStream) -> TokenStream {
    ...
    for field in &named.named {
        ...
        if strategy.ignore.is_some() {
            ...
            fields.push(quote! { .field(#name, &#runtime::__private::mask_str(&format!("{:?}", self.#field_ident))) });
            ...
            fields.push(quote! { .field(#name, &#runtime::__private::limit_str(&format!("{:?}", self.#field_ident), #limit)) });
        } else {
