...
2. 使用显式工具链命令执行本地检查，避免 `rust-toolchain.toml` 掩盖 stable 验证：
   ...
   cargo +1.97.0 check --locked -p fusen-common --no-default-features
...
5. 从干净环境运行不依赖 Nacos/OTLP 的 Direct 示例。真实 Nacos 验证是可选的手工步骤，由维护者准备服务并显式运行：
   ...
   ```shell
   NACOS_ADDR=127.0.0.1:8848 cargo +1.97.0 test --locked -p fusen-common --all-features live_nacos_registration_when_configured -- --ignored
   ```
