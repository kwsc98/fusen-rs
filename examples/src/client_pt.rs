...
use fusen::{client::FusenClient, fusen_client, fusen_common, register::{RegisterBuilder, RegisterType}};
...
async fn do_run(client : TestServer , sender : mpsc::Sender<i32>) {
    for _idx in 0..100000 {
        ...
        tokio::spawn(async move {
            let res = temp_client.doRun2(ReqDto{str : "client say hello 1".to_string()}).await;
            info!("{:?}",res);
            drop(temp_sender);
        });
    }
