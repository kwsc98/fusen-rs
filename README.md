...
- The stable extension surface is limited to `Interceptor`, `Registry`, `ConfigSource`, `InstanceRouter`, `LoadBalancer`, `RetryPolicy`, `MetricsRecorder`, and the client-side `RequestEncoder`/`ResponseDecoder`/`ErrorDecoder` binding codecs.
...
`http-json-v1` maps every service method to its declared HTTP operation. JSON request fields and raw JSON success responses use `application/json` by default; `consumes` and `produces` may override the media types on the method declaration.
